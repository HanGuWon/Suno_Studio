...
9. Confirm or change the result family (`mix`, `stems`, `tempo-locked stems`, `MIDI`), then use `Import Dropped`.
10. You can cancel any family picker; the client skips `/manual-complete` unless at least one file was selected.
11. Client calls `/jobs/{job_id}/manual-complete`, then selects the first available output for reveal/drag.
12. Use the output selector to choose a different file, then reveal it or drag/copy it into the DAW.
